...
function ChatInner() {
  ...
  useEffect(()=>{
    ...
    fetch("http://192.168.1.23:3000/getconversation", {
       method : "POST",
       headers : {
      ...
      console.log("here is the reply from the backend, the conversationid: ", res._id)
  ...
  function sendMessage(){
    console.log("here is the text:- ", text)
     ...
     fetch("http://192.168.1.23:3000/send-message", {
      method : "POST",
      headers : {
        "Content-Type" : "application/json",
        Authorization: `Bearer ${token} `
    ...
    <div className="min-h-screen bg-[#F7F7F7] p-3 md:p-4">
      <div className="max-w-[1700px] mx-auto flex gap-4 h-[calc(100vh-24px)] relative">
        ...
        <main className="flex-1 flex flex-col bg-white rounded-[28px] border border-gray-200 shadow-sm overflow-hidden">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6"></div>
          ...
          <div className="border-t border-gray-100 p-4 md:p-6">
            <div className="flex gap-3">
              ...
              <button onClick={sendMessage}  className="bg-violet-500 hover:bg-violet-600 text-white px-5 rounded-xl transition">
                <Send size={18} />
              </button>
            </div>
